...
class Student:
    def __init__(self, name):
        self.name = name
        self.times_tutored = 0
        self.classes_tutored = []
        self.days_attended = []
        self.session_durations = []

    def append(self, date, duration, class_id):
        if(class_id not in self.classes_tutored):
            self.classes_tutored.append(class_id)
        if(date.date() not in self.days_attended):
            self.days_attended.append(date.date())
        self.session_durations.append(duration)
        self.times_tutored += 1
    ...
    def __repr__(self):
        return "<Student: {} (got help {} times) (days attended: {}) (avg help time: {}), (classes: {})>".format(self.name, self.times_tutored, len(self.days_attended), self.average_session_length(), ', '.join(self.classes_tutored))
...
class Record:
    def __init__(self,
        ...
        # Check if the student table has the student already, else generate it
        student_name = student.title().strip()

        if(student_name == '' or student_name is None):
            raise ValueError('Empty student name on line {}!'.format(line_no))

        student_table_entry = students_table.get(student_name)
        if(student_table_entry is None):
            self.student = Student(student_name)
            students_table[student_name] = self.student
        else:
            self.student = student_table_entry
        self.student.append(self.date, self.out_time - self.in_time, class_id)
...
class RecordsSheet:
    ...
    def __repr__(self):
        return "<Records Sheet ({} entries) ({} unique tutors) ({} unique students)>" \
               .format(len(self.records), len(self.tutors), len(self.students))
